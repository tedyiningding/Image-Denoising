...
function main
    ...
    fprintf('Spectral Norm denoised image: RSNR = %.4f dB\n',calcRSNR(xsol,x0));

end


function [x, primal_cost, dual_cost] = TSVdenoising(y,lambda,tau,Nbiter)
	...
	for iter = 1:Nbiter
		...
		x2 = x2+rho*(x-x2);
		u2 = u2+rho*(u-u2);
        primal_cost(iter) = sum(sum(sum((x-y).^2)))/2 + lambda*spcnorm(opD(x));
...
function u = prox_sigma_g_conj(y, lambda)
    ...
    x = cat(4,bsxfun(@times,y(:,:,:,1),c)+bsxfun(@times,y(:,:,:,2),s),...
		bsxfun(@times,y(:,:,:,2),c)-bsxfun(@times,y(:,:,:,1),s));               % MxNx3x2 (U*S)
    S = sqrt(sum(x.^2,3));                      % MxNx1x2 (the norm of the two columns are the two sigular values)
    tmp = projball_l1(S, lambda);         % MxNx1x2
...
function x = projball_l1(y, tau)
    % adapted from L.Condat's implementation projball_l1 in
    % https://lcondat.github.io/download/Condat_proxs.m
	tmp = abs(y);        % MxNx1x2
    lambda = max((cumsum(sort(tmp, 4, 'descend'), 4) - tau) ./ reshape(1:2, [1,1,1,2]), [], 4); % MxN
    x = y - max(min(y, lambda), -lambda);   % MxNx1x2
